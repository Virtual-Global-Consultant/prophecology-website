<script setup>
import DefaultProgramStepComponent from '@/components/Dashboard/Programs/DefaultProgramStepComponent.vue'
...
Api.getData(ApiEndpoints.getProgram(route.params.id)).then(res => {
  const data = res.data;
  if (data.success) {
    stepData.value = data.data.steps[0]
    stepAppState.value = appState().loaded
    stepErrorMessage.value = ''
  }
}).catch(error => {
  stepAppState.value = appState().error
  stepErrorMessage.value = ''
  errorLogger(error.response.data)
});
</script>
<template>
  <div v-if="stepAppState == appState().loading" class=" flex justify-center items-center w-full">
          <span>
  ...
  <DefaultProgramStepComponent v-if="stepAppState == appState().loaded"
    ...
    <template v-slot:programTemplate>
      <div class="px-10 mt-5 space-y-3">
              ...
              <label class="flex items-center text-sm">
                ...
                Main Hotel: The LaGuardia Plaza Hotel 104-04Blvd., East Elmhurst, NY 11369 </label
              ><label class="flex items-center text-sm">
                <input type="radio" class="mr-4" />
                Secondary Hotel: The Hampton Inn New York LaGuardia Airport Hotel 102-40 Ditmars
                Boulevard, Queens, NY, 11369, US </label
              ><label class="flex items-center text-sm text-dim-gray">
                <input type="radio" class="mr-4" />
                I'll be staying somewhere else
              </label>
            </div>
            <div
              class="flex flex-col space-y-4 md:space-y-0 md:flex-row items-center mt-6 md:space-x-5"
            >
